...
motivationForm.onsubmit = (event) => {
  ...
  axios
    .post(`${baseURL}/motivation`, bodyObj)
    .then((res) => {
      displayMotivationEntry(res.data);
    })
    .catch((err) => {
      console.log({ error: err });
    });
...
const displayMotivationEntry = (entry) => {
  const motivationEntry = document.createElement("div");
  motivationEntry.innerHTML = `<h2>${entry}`;
  motivationContainer.appendChild(motivationEntry);
};
const login = (body) =>
  axios
